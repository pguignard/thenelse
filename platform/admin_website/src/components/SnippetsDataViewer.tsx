...
function formatLLMResponseForTextArea(response_content: string | '') {
    ...
    let formattedContent = response_content || 'Impossible d’afficher le contenu.';
    try {
...
export function SnippetsDataViewer({ responseContent }: SnippetsDataViewerProps) {
    ...
    return (
        <div>
            ...
            {mode === 'snippets' && snippetList && Array.isArray(snippetList) && (
                <div> {batchDetails && (
                    <div style={{ marginBottom: '1rem' }}>
                        <strong>Détails du batch :</strong><br />
                        Nombre de snippets : {batchDetails.snippet_count}<br />
                        Langage : {batchDetails.language}<br />
                        Niveau : {batchDetails.level}<br />
                        Thème : {batchDetails.theme}
                    </div>
                )}
                    <div>
                        {snippetList.map((item, index) => (
                            <div key={index} style={{ marginBottom: '2rem', padding: '1rem', border: '1px solid #ccc', borderRadius: '5px' }}>
                                <div style={{ marginBottom: '0.5rem' }}>
                                    <strong>Snippet {index + 1}:</strong><br />
            ...
            {mode === 'snippets' && (!snippetList || !Array.isArray(snippetList)) && (
                <div>Impossible d’afficher les snippets (le contenu n’est pas un tableau JSON valide).</div>
            )}
        </div>
    );
}
