...
export async function fetchRequestHistoryFolderContent(
  folderName: string
): Promise<RHFolderContentResponse> {
  const url = `/get_request_history_folder_content?folder_name=${encodeURIComponent(
    folderName
  )}`;
...
export async function fetchRequestInformation(
  fileName: string,
  folderName: string
): Promise<RequestInformationsResponse> {
  const url = `/get_request_information?file_name=${encodeURIComponent(
    fileName
